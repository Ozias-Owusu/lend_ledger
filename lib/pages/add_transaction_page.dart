...
// import 'package:lend_ledger/models/customer.dart';import 'package:lend_ledger/models/loan_record.dart';
...
//
//   // --- NEW: State for Soft Loan Duration ---
//   int _loanDurationInMonths = 3; // Default to 3 months
//
...
//       // For both loan types, the total interest is calculated once on the principal.
//       final interest = amount * (_interestPercent / 100);
//       finalAmount = amount + interest;
//       interestToSave = _interestPercent;
//
//       // Add duration to the note for soft loans
//       if (loanKind == 'soft') {
//         note = 'Duration: $_loanDurationInMonths months';
//       }
//     }
//
//     if (_type == TransactionType.repayment) {
...
//
//     // Calculate monthly installment ONLY for soft loans
//     final monthlyInstallment = isSoftLoan && _loanDurationInMonths > 0 ? totalRepayable / _loanDurationInMonths : 0.0;
//
//     // Calculate end date ONLY for soft loans
//     final loanEndDate = isSoftLoan ? DateFormat('dd MMM, yyyy').format(DateTime.now().add(Duration(days: 30 * _loanDurationInMonths))) : '';
//
//     return Scaffold(
//       appBar: AppBar(title: Text(title)),
//       body: SingleChildScrollView(
//         padding: const EdgeInsets.all(16),
//         // *** THE FIX IS HERE: A single top-level Column ***
//         child: Column(
...
//                   value: _loanDurationInMonths,
//                   items: List.generate(12, (index) => index + 1).map((months) => DropdownMenuItem(value: months, child: Text('$months Month${months > 1 ? 's' : ''}'))).toList(),
//                   onChanged: (value) => setState(() => _loanDurationInMonths = value!),
...
//                         _summaryRow("Monthly Installment:", "GHS ${monthlyInstallment.toStringAsFixed(2)}"),
//                         const Divider(height: 20),
